...
public class MSCameraController : MonoBehaviour {
	...
	public GameObject playerCamsObj;
	...
	void ManageCameras(){
		...
		case MSACC_CameraType.TipoRotac.FirstPerson:
			...
			if (CameraSettings.firstPerson.rotateWhenClick) {
				if (ControlFreak2.CF2Input.GetKey (CameraSettings.firstPerson.keyToRotate) || _enableMobileInputs) {
		...
		case MSACC_CameraType.TipoRotac.Orbital:
			...
			if (CameraSettings.orbital.rotateWhenClick) {
				if (ControlFreak2.CF2Input.GetKey (CameraSettings.orbital.keyToRotate) || _enableMobileInputs) {
		...
		case MSACC_CameraType.TipoRotac.OrbitalThatFollows:
			...
			if (CameraSettings.OrbitalThatFollows.rotateWhenClick) {
				if (ControlFreak2.CF2Input.GetKey (CameraSettings.OrbitalThatFollows.keyToRotate) || _enableMobileInputs) {
			...
			case MSACC_SettingsCameraOrbitalThatFollows.ResetTimeType.Input_OnlyWindows:
				if (ControlFreak2.CF2Input.GetKeyDown(CameraSettings.OrbitalThatFollows.resetKey)) {
					orbitalAtiv = false;
					orbital_AtivTemp = false;
				}
				break;
		...
		case MSACC_CameraType.TipoRotac.ETS_StyleCamera:
			...
			if (CameraSettings.ETS_StyleCamera.rotateWhenClick) {
				if (ControlFreak2.CF2Input.GetKey (CameraSettings.ETS_StyleCamera.keyToRotate) || _enableMobileInputs) {
		...
		case MSACC_CameraType.TipoRotac.FlyCamera_OnlyWindows:
			...
			if (ControlFreak2.CF2Input.GetKey (CameraSettings.FlyCamera_OnlyWindows.speedKeyCode)) {
				speedCamFly *= 3.0f;
			}
			cameras [index]._camera.transform.position += cameras [index]._camera.transform.right * speedCamFly * ControlFreak2.CF2Input.GetAxis(CameraSettings.FlyCamera_OnlyWindows.horizontalMove) * Time.deltaTime;
			cameras [index]._camera.transform.position += cameras [index]._camera.transform.forward * speedCamFly * ControlFreak2.CF2Input.GetAxis(CameraSettings.FlyCamera_OnlyWindows.verticalMove) * Time.deltaTime;
			//
			if(ControlFreak2.CF2Input.GetKey(CameraSettings.FlyCamera_OnlyWindows.moveUp)){
				cameras [index]._camera.transform.position += Vector3.up * speedCamFly * Time.deltaTime;
			}
			if(ControlFreak2.CF2Input.GetKey(CameraSettings.FlyCamera_OnlyWindows.moveDown)){
				cameras [index]._camera.transform.position -= Vector3.up * speedCamFly * Time.deltaTime;
			}
			break;
		}
	...
	void Update(){
		...
		if (!_enableMobileInputs) {
			_horizontalInputMSACC = ControlFreak2.CF2Input.GetAxis (CameraSettings.inputMouseX);
			_verticalInputMSACC = ControlFreak2.CF2Input.GetAxis (CameraSettings.inputMouseY);
			_scrollInputMSACC = ControlFreak2.CF2Input.GetAxis (CameraSettings.inputMouseScrollWheel);
		} else {
		...
		if (!_enableMobileInputs) {
			if (Time.timeScale > 0) {
				if (ControlFreak2.CF2Input.GetKeyDown (CameraSettings.cameraSwitchKey) && index < (cameras.Length - 1)) {
					lastIndex = index;
					index++;
					EnableCameras (index);
				} else if (ControlFreak2.CF2Input.GetKeyDown (CameraSettings.cameraSwitchKey) && index >= (cameras.Length - 1)) {
					lastIndex = index;
					index = 0;
					EnableCameras (index);
				}

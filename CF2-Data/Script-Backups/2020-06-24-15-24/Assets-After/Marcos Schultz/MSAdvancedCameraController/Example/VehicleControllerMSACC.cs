...
public class VehicleControllerMSACC : MonoBehaviour {
	...
	void Update () {
		...
		direction = ControlFreak2.CF2Input.GetAxis ("Horizontal");
		...
		if (Mathf.Abs (ControlFreak2.CF2Input.GetAxis("Vertical")) < 0.1f) {
			motorTorque = 0.0f;
			brakeTorque = Mathf.Lerp (brakeTorque, rb.mass, Time.deltaTime * 2.0f);
		} else {
			motorTorque = Mathf.Lerp (motorTorque, ControlFreak2.CF2Input.GetAxis ("Vertical") * rb.mass * torqueForceWheel, Time.deltaTime);
			brakeTorque = 0.0f;
		}

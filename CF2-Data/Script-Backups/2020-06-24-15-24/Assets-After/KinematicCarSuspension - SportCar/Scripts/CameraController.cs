	...
	void Update () {
		...
		case(0):
			...
			if (ControlFreak2.CF2Input.GetKey (KeyCode.Mouse1)) {
				rotX += ControlFreak2.CF2Input.GetAxis ("Mouse X") * MouseSpeed;
				rotY -= ControlFreak2.CF2Input.GetAxis ("Mouse Y") * MouseSpeed;
			}
			rotY = Mathf.Clamp (rotY, 0, 70);
			DistanceCam -= ControlFreak2.CF2Input.GetAxis ("Mouse ScrollWheel") * MouseScrollSpeed;

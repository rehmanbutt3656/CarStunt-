...
public class SimpleCarController : MonoBehaviour {
	...
	void GetSmoothSteeringInput(){
		SmoothSteeringInput = Mathf.MoveTowards (SmoothSteeringInput, ControlFreak2.CF2Input.GetAxisRaw ("Horizontal"), SteeringSpeed * Time.fixedDeltaTime);
	}
	...
	void Update () {
		...
		foreach (var wheel in Wheels) {
			...
			if (wheel.Motor && wheel.wheelCollider!=null) wheel.wheelCollider.motorTorque = ControlFreak2.CF2Input.GetAxis ("Vertical") * MotorPower;

			if (wheel.Steering && wheel.wheelCollider != null) 
				wheel.wheelCollider.steerAngle = SmoothSteeringInput * SteeringAngle;

			if (Mathf.Sign (ControlFreak2.CF2Input.GetAxis ("Vertical")) != Mathf.Sign (wheel.wheelCollider.rpm) 
				&& Mathf.Abs(wheel.wheelCollider.rpm)>20 
				&& ControlFreak2.CF2Input.GetAxis("Vertical")!=0) 

...
//			wheels.wheelFR.steerAngle = maxSteer * steerval;//steerInput;
	...
	void DriveMobileTest()
	{
		...
		//when the gas button is released, slow the car down
		else if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.I)) {
			throttleInput = 0;
			gasPedal.color = Color.white;
		}
		//now the same thing for the brakes
		...
		//stop braking once you put your finger off the brake pedal
		else if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.K)) {
			throttleInput = 0;
			brakePedal.color = Color.white;
		}
		//now the left steering column...
		...
		//and stop the steering once you take your finger off the turn button
		else if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.J)) {
			currentLerpTime = 0f;
			steerInput = 0;
			DriveButtonTo = 0;
			leftPedal.color = Color.white;
		...
		//and stop the steering once you take your finger off the turn button
		else if (ControlFreak2.CF2Input.GetKeyUp(KeyCode.L)) {
			currentLerpTime = 0f;
			steerInput = 0; 
			DriveButtonTo = 0;
			rightPedal.color = Color.white;
	...
	void Drive()
	{
		...
		if (DriveTrain == DriveType.RWD)
		{
			wheels.wheelRR.motorTorque = maxTorque * ControlFreak2.CF2Input.GetAxis("Vertical") * gasMultiplier;
			wheels.wheelRL.motorTorque = maxTorque * ControlFreak2.CF2Input.GetAxis("Vertical") * gasMultiplier;
		...
		if (DriveTrain == DriveType.FWD)
		{
			wheels.wheelFL.motorTorque = maxTorque * ControlFreak2.CF2Input.GetAxis("Vertical") * gasMultiplier;
			wheels.wheelFR.motorTorque = maxTorque * ControlFreak2.CF2Input.GetAxis("Vertical") * gasMultiplier;
...
//		wheels.wheelFL.steerAngle = maxSteer * steerval;//steerInput;
//		wheels.wheelFR.steerAngle = maxSteer * steerval;//steerInput;
		if (ControlFreak2.CF2Input.GetButton("Jump"))//pressing space triggers the car's handbrake
		{
			wheels.wheelFL.brakeTorque = handBrakeTorque;
			wheels.wheelFR.brakeTorque = handBrakeTorque;
			wheels.wheelRL.brakeTorque = handBrakeTorque;

...
public class SmoothOrbitCam : MonoBehaviour
{
	...
	void CalcPan ()
	{
		...
		if (UiBlocksInteraction) {
			if (!uiBlocking) {
				velocityPanX += panSpeed * ControlFreak2.CF2Input.GetAxis ("Mouse X") * 0.2f;
				velocityPanY += panSpeed * ControlFreak2.CF2Input.GetAxis ("Mouse Y") * 0.2f;
			}
		} else {
			velocityPanX += panSpeed * ControlFreak2.CF2Input.GetAxis ("Mouse X") * 0.2f;
			velocityPanY += panSpeed * ControlFreak2.CF2Input.GetAxis ("Mouse Y") * 0.2f;
	...
	void LateUpdate ()
	{
		...
		if (target) {
			if (!useTouch) {
				...
				if (ControlFreak2.CF2Input.GetKey (orbitKey)) {
					doOrbit = true;
				} else {
					doOrbit = false;
				}
				...
				if (doOrbit) {
					if (UiBlocksInteraction) {
						if (!uiBlocking) {
							//on mouse down, calculate the orbital velocity with the given speed values and the axes
							if (!limitToXAxis)
								velocityX += xSpeed * ControlFreak2.CF2Input.GetAxis ("Mouse X") * 0.2f;
							if (!limitToYAxis)
								velocityY += ySpeed * ControlFreak2.CF2Input.GetAxis ("Mouse Y") * 0.2f;
						}
					} else {
						//on mouse down, calculate the orbital velocity with the given speed values and the axes
						if (!limitToXAxis)
				...
				if (enableZooming) {
					distance = Mathf.Clamp (distance - ControlFreak2.CF2Input.GetAxis ("Mouse ScrollWheel") * zoomSpeed * 5, distanceMin,
						distanceMax);
				}
				...
				if (ControlFreak2.CF2Input.touchCount == 1) {
					if (UiBlocksInteraction) {
						if (!uiBlocking) {
							...
							if (!limitToXAxis)
								velocityX += xSpeed * ControlFreak2.CF2Input.GetTouch (0).deltaPosition.x *
								(Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.01f;
							if (!limitToYAxis)
								velocityY += ySpeed * ControlFreak2.CF2Input.GetTouch (0).deltaPosition.y *
								(Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.01f;
						}
					} else {
						//on touch down calculate the velocities from the input touch position and the modifyers
			...
			if (!useTouch) {
				...
				if (useable) {
					if (ControlFreak2.CF2Input.GetKey (panKey) && enablePanning) {
						CalcPan ();
					}
				}
			...
			if (useTouch) {
				//for touch:
				if (ControlFreak2.CF2Input.touchCount == 2 && enablePanning && useable && panKey != KeyCode.Mouse0) {

					//on touch down calculate the velocities from the input touch position and the modifyers
					float tempVeloXa = panSpeed * ControlFreak2.CF2Input.GetTouch (0).deltaPosition.x *
					                            (Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.05f;
					float tempVeloXb = panSpeed * ControlFreak2.CF2Input.GetTouch (1).deltaPosition.x *
					                            (Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.05f;
					float tempVeloYa = panSpeed * ControlFreak2.CF2Input.GetTouch (0).deltaPosition.y *
					                            (Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.05f;
					float tempVeloYb = panSpeed * ControlFreak2.CF2Input.GetTouch (1).deltaPosition.y *
					                            (Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.05f;
				...
				if (ControlFreak2.CF2Input.touchCount == 1 && enablePanning && useable && panKey == KeyCode.Mouse0) {
					float veloDeltaX = panSpeed * ControlFreak2.CF2Input.GetTouch (0).deltaPosition.x *
					                            (Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.05f;
					float veloDeltaY = panSpeed * ControlFreak2.CF2Input.GetTouch (0).deltaPosition.y *
					                            (Time.deltaTime / (ControlFreak2.CF2Input.GetTouch (0).deltaTime + 0.001f)) * 0.05f;

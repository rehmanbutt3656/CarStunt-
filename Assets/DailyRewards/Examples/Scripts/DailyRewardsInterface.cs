...
namespace NiobiumStudios
{
    ...
    public class DailyRewardsInterface : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            int totalCash = PlayerPrefs.GetInt("AvailableCash", 0);
            CurrentcoinText.text = totalCash.ToString();
            if (panelDebug)
            {
        ...
        {
            if (dailyRewards.lastReward < 7) { PlayerPrefs.SetInt("RewardWeek", 1); }
            else if (dailyRewards.lastReward >= 7 && dailyRewards.lastReward < 14) { PlayerPrefs.SetInt("RewardWeek", 2); }
            else if (dailyRewards.lastReward >= 14 && dailyRewards.lastReward < 21) { PlayerPrefs.SetInt("RewardWeek", 3); }
            else if (dailyRewards.lastReward >= 21) { PlayerPrefs.SetInt("RewardWeek", 4); }

            int week = PlayerPrefs.GetInt("RewardWeek", 1);

            if (MainMenuController.Instance.awakeCallforDailyRewards)
            {
                ///Seven Days Reward UI here
                for (int i = 0; i < dailyRewards.rewards.Count; i++)
                {
                    if (week == 1 && i < 7) { SetUI(i); }
                    else if (week == 2 && i >= 7 && i < 14) { SetUI(i); }
                    else if (week == 2 && i >= 14 && i < 21) { SetUI(i); }
                    else if (week == 2 && i >= 21) { SetUI(i); }
                }
                ///
            }
            else
        ...
        private void CheckTimeDifference()
        {
            if (!readyToClaim)
            {
                ...
                if (MainMenuController.Instance.awakeCallforDailyRewards)
                {
                    MainMenuController.Instance.awakeCallforDailyRewards = false;
                    Destroy(GameObject.Find("7DaysReward Landscape(Clone)"));
                }
            }
        }
        ...
        private void OnClaimPrize(int day)
        {
            ...
            int totalCash = PlayerPrefs.GetInt("AvailableCash", 0);
            totalCash += reward.reward;
            PlayerPrefs.SetInt("AvailableCash", totalCash);
            CurrentcoinText.text = totalCash.ToString();
        }

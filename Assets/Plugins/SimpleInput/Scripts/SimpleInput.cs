...
public class SimpleInput : MonoBehaviour
{
	...
	private static void TrackAxis( string axis, bool trackUnityAxisOnly = false )
	{
		try
		{
			AxisInput unityAxis = new AxisInput( axis ) { value = ControlFreak2.CF2Input.GetAxisRaw( axis ) };
	...
	private static void TrackButton( string button, bool trackUnityButtonOnly = false )
	{
		try
		{
			ButtonInput unityButton = new ButtonInput( button ) { value = ControlFreak2.CF2Input.GetButton( button ) };
	...
	private static void TrackMouseButton( int button, bool trackUnityMouseButtonOnly = false )
	{
		try
		{
			MouseButtonInput unityMouseButton = new MouseButtonInput( button ) { value = ControlFreak2.CF2Input.GetMouseButton( button ) };
	...
	private void Update()
	{
		...
		for( int i = 0; i < trackedUnityAxes.Count; i++ )
			trackedUnityAxes[i].value = ControlFreak2.CF2Input.GetAxisRaw( trackedUnityAxes[i].Key );

		for( int i = 0; i < trackedUnityButtons.Count; i++ )
			trackedUnityButtons[i].value = ControlFreak2.CF2Input.GetButton( trackedUnityButtons[i].Key );

		for( int i = 0; i < trackedUnityMouseButtons.Count; i++ )
			trackedUnityMouseButtons[i].value = ControlFreak2.CF2Input.GetMouseButton( trackedUnityMouseButtons[i].Key );
